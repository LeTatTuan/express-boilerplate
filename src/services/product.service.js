...
class productService {
  ...
  static updateProduct = async (id, userId, productDto) => {
    ...
    if (product.user.toString() !== userId) throw new Unauthorized(`Don't permision update this product`);
  ...
  static deleteProduct = async (id, userId) => {
    const product = await Product.findOne({ _id: id });
    if (!product) throw new NotFoundError(`No product with id: ${id}`);
    if (product.user.toString() !== userId) throw new Unauthorized(`Don't permision delete this product`);
